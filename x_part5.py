...
SHOP = [
  ...
  {"id":"rename_token", "name":"Rename Token", "price":120, "token":True},
  {"id":"event_pass", "name":"Event Pass", "price":200, "pass":True}
]
...
class ShopCog(commands.Cog):
    ...
    async def buy(self, inter: discord.Interaction, item_id: str):
        ...
        if it.get("count"):
            ...
            return await inter.response.send_message(embed=theme_embed("🕯️ Incense", f"{it['name']} activated in this channel.", "success"))
        await inter.response.send_message(embed=theme_embed("🎟️ Purchased", f"You bought {it['name']}. Perks unlocked!", "success"))

async def setup(bot): await bot.add_cog(ShopCog(bot))
''')

print("bootstrap_x Part 5 applied: shop.py written.")
